...
class Solution(object):
    ...
    def maxPathSum(self, root):
        ...
        return self.inorderTraverse(maxTree)
...
#       Set value as infinity instead of 0 otherwise max will take 0 instead of negative numbers
    ...
    def inorderTraverse(self, maxTree, maxSum = float("-inf")):
        if maxTree is None:
            return None
        maxSum = max(maxTree.val)
        return max(maxSum, self.inorderTraverse(maxTree.left, maxSum), self.inorderTraverse(maxTree.right, maxSum))
